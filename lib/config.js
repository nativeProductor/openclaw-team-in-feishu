...
export function resolveConfigPath(givenPath) {
  ...
  const candidates = [
    "./octf.json",
    "./openclaw-team-in-feishu.json",
    path.join(process.env.HOME || "", ".config/octf/config.json"),
  ];
  for (const p of candidates) if (fs.existsSync(p)) return p;
  throw new Error("no config found; pass --config <path> or create ./octf.json");
}
...
function applyDefaults(c) {
  ...
  if (!c.transcriptDir) {
    // Default under openclawRoot if available, else /var/lib/octf/shared.
...
function validate(c) {
  ...
  if (!Array.isArray(c.chats)) {
    throw new Error("config.chats must be an array");
  }
  // Empty chats[] is OK during bootstrap (init has run, chats not yet bound).
  // The daemon's runDaemon() does its own per-chat work and handles empty
  // gracefully — no need to enforce non-empty at config-load time, and doing
  // so blocks `octf chat add` from working when removing the last chat first.
  // Agents can serve MULTIPLE chats — same agent (and Feishu app) may appear
  // in chats[].host or chats[].members across different chats. SOUL.md is now
  // chat-agnostic (transcript injected via prompt at invoke time), so this
  // works as long as the bot is a member of every chat it serves.
  ...
  for (const [i, ch] of c.chats.entries()) {
    ...
    if (chatIds.has(ch.chatId)) throw new Error(`${where}: chatId "${ch.chatId}" duplicated across chats[]`);
    chatIds.add(ch.chatId);
    ...
    for (const m of ch.members) {
      ...
      if (m === ch.host) throw new Error(`${where}: member "${m}" cannot also be the host within the same chat`);
    }
