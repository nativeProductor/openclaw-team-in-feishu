...
export function preflight(config, openclawBin) {
  ...
  // Check renderMode for member bots: must be "raw" so OpenClaw's Feishu plugin
  // emits plain text rather than auto-rendering long replies as interactive
  // cards. Card payloads are technically extractable (see extractText), but
  // pinning renderMode=raw at the source is more reliable.
  const bin = openclawBin || config.openclawBin || "openclaw";
  for (const appId of memberApps) {
    let getOut;
    try {
      getOut = execFileSync(bin,
        ["config", "get", `channels.feishu.accounts.bot-${appId}`],
        { stdio: ["ignore", "pipe", "pipe"], maxBuffer: 1024 * 1024 }).toString();
    } catch { continue; }
    let acc;
    try {
      const m = getOut.match(/\{[\s\S]*\}/);
      acc = m ? JSON.parse(m[0]) : null;
    } catch { acc = null; }
    if (!acc) continue;
    // renderMode must be explicitly "raw". Both undefined (default = auto) and
    // any non-raw explicit value need fixing — undefined was historically
    // skipped here, which masked the most common new-user state.
    if (acc.renderMode !== "raw") {
      const current = acc.renderMode == null ? "(unset; defaults to auto)" : `"${acc.renderMode}"`;
      reasons.push(`member ${appId}: renderMode is ${current}, should be "raw" so replies are plain text\n  fix: openclaw config set channels.feishu.accounts.bot-${appId}.renderMode raw`);
    }
  }
