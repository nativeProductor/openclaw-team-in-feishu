...
export function runDaemon(config, { onLog = console.log, onError = console.error } = {}) {
  ...
  async function invokeRoundRobinMember(chatId, tid, memberAppId, replyToMsgId) {
    ...
    try {
      ...
      const reply = runOpenclaw(memberApp.agent, prompt);
      ...
      if (sender?.id_type === "app_id" && sender?.id === cfg.hostApp) {
        ...
        if (cfg.mode === "round-robin" && msg.mentions) {
          for (const mention of msg.mentions) {
            const matchedAppId = cfg.memberApps.find(appId => {
              const a = apps[appId];
              return mention.name === a.botName ||
                     (a.botName && (mention.name || "").includes(a.botName));
            });
            if (matchedAppId) {
              onLog(`[trigger RR] tid=${tid.slice(-8)} | host @${apps[matchedAppId].role} → invoke member`);
              invokeRoundRobinMember(chatId, tid, matchedAppId, msg.message_id);
              break; // honor "one @ per host message" SOUL convention
            }
          }
        }
        continue;
