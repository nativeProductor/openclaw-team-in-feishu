...
// poll, write transcript, fork openclaw, post replies.
// version — see "How it works" in README.
...
// bots enabled-and-running. This is the
...
export function preflight(config, openclawBin) {
  ...
  for (const appId of memberApps) {
    ...
    // Daemon now drives member dispatch directly (no longer relying on
    // OpenClaw's native lark plugin). Both host AND member bots must have
    // their native plugin DISABLED so daemon owns all replies.
    if (!/disabled/.test(line)) {
      reasons.push(`member ${appId}: must be DISABLED in openclaw config (daemon drives all dispatch)\n  fix: openclaw config set channels.feishu.accounts.bot-${appId}.enabled false`);
    }
  }
  // renderMode preflight is no longer needed: daemon posts directly to Feishu
  // API (not via openclaw plugin), so openclaw's renderMode setting doesn't
  // affect the messages daemon emits.
...
export function runDaemon(config, { onLog = console.log, onError = console.error } = {}) {
  ...
  onLog(`[preflight] ✓ openclaw channels: ${pre.hostCount} hosts + ${pre.memberCount} members all disabled (daemon drives all dispatch)`);
  ...
  // Per-agent global mutex: serialize all `openclaw agent --agent X` invocations
  // for the same agent name across all chats and threads. Without this, when
  // the same agent serves multiple chats (v0.1.2+), two chats can simultaneously
  // invoke the same OpenClaw agent process, corrupting its session memory or
  // letting concurrent stdout streams interleave. Trade-off: cross-chat
  // responses for that single agent become serial; different agents still
  // parallelize freely. The Map grows to O(unique agent names) — bounded.
  const agentLocks = new Map(); // agentName -> Promise (queue tail)
  async function withAgentLock(agentName, fn) {
    const prev = agentLocks.get(agentName) || Promise.resolve();
    let release;
    const current = new Promise(r => { release = r; });
    agentLocks.set(agentName, prev.then(() => current));
    await prev;
    try { return await fn(); }
    finally { release(); }
  }
  ...
  function ensureThread(chatId, tid) {
    if (!state[chatId].threads[tid]) {
      state[chatId].threads[tid] = {
        ...
        lastSpeakerAt: 0,
        messageCount: 0,
        consecutiveAllSkipped: 0, // free-speak: tracks N rounds where every candidate SKIPped
      };
  ...
  async function kickoffFromMain(chatId, userMsg) {
    ...
    try {
      ...
      const reply = await withAgentLock(cfg.hostAgent, () => runOpenclaw(cfg.hostAgent, prompt));
  ...
  // ─── round-robin: invoke a member after host @-mentions them ─────────
  // Replaces the old behavior of relying on OpenClaw's native lark plugin
  // to auto-fire on @-mention events. Now daemon owns the full dispatch:
  // pollThread detects host's @, looks up which member by botName, calls
  // openclaw CLI, and posts the reply as that member's bot. This unifies
  // round-robin and free-speak member dispatch under daemon control, which
  // is what makes multi-chat per agent possible (transcript is injected
  // per-invocation, no longer hardcoded in SOUL.md).
  async function invokeRoundRobinMember(chatId, tid, memberAppId, replyToMsgId) {
    const cfg = groups[chatId];
    ...
    try {
      ...
      const memberApp = apps[memberAppId];
      const prompt =
        `# 当前讨论 transcript（按时间顺序）\n\n${transcript}\n\n` +
        `# 你的任务\n你是「${memberApp.role}」。主持人 (${cfg.hostRole}) 刚 @ 了你让你发言。\n` +
        `基于上面 transcript 和你的 SOUL.md，给一个简短的发言（≤ 5 句），覆盖你的核心专业视角。\n` +
        `**不要 @ 任何人**——让主持人调度。直接给纯文本输出，不要 meta 解释。`;
      onLog(`[invoke RR member] ${memberApp.agent} tid=${tid.slice(-8)} (${transcript.length}b)`);
      const reply = await withAgentLock(memberApp.agent, () => runOpenclaw(memberApp.agent, prompt));
      if (!reply) { onError(`[invoke RR member ${memberApp.agent}] empty`); return; }
      const posted = await postReplyAs(memberAppId, replyToMsgId, reply);
      if (posted) {
        tst.lastSpeakerAppId = memberAppId;
        tst.lastSpeakerAt = Date.now();
        tst.messageCount++;
      }
    } catch (e) {
      onError(`[invoke RR member] ${e.message}`);
    } finally {
      tst.invoking = false;
    }
  }
  ...
  async function tryFreeSpeakStep(chatId, tid) {
    ...
    try {
      ...
      for (const [idx, appId] of candidates.entries()) {
        ...
        // Anti-deadlock: if everyone SKIPped 2+ rounds in a row, force host
        // out of SKIP. This prevents the failure mode where every agent
        // (host included) defers to others and discussion freezes.
        const stuck = isHost && tst.consecutiveAllSkipped >= 2;
        const stuckNotice = stuck
          ? `⚠️ **反僵局警告（你必须处理）**：连续 ${tst.consecutiveAllSkipped} 轮所有成员都 SKIP 了。你不能再 SKIP——本次必须二选一：(a) 用纯文本 steer 一个具体新角度让讨论展开；(b) 基于已有信息直接收口 + ${cfg.modeOptions.endKeyword}。\n`
          : "";
        ...
        const collabState =
          ...
          `- 连续全员 SKIP 计数：${tst.consecutiveAllSkipped}\n` +
          stuckNotice +
        ...
        let reply;
        try { reply = await withAgentLock(apps[appId].agent, () => runOpenclaw(apps[appId].agent, prompt)); }
        ...
        tst.lastSpeakerAt = Date.now();
        tst.messageCount++;
        tst.consecutiveAllSkipped = 0; // someone spoke — reset stuck counter
      ...
      // All candidates skipped this iteration. Increment counter; once it
      // hits 2+ the host gets a stuckNotice in its prompt next time and
      // will be forced to wrap.
      tst.consecutiveAllSkipped++;
      onLog(`[FS all-skip count=${tst.consecutiveAllSkipped}] tid=${tid.slice(-8)}`);
    } finally {
      tst.invoking = false;
    }
  }
  ...
  async function pollThread(chatId, tid) {
    ...
    for (const msg of items) {
      ...
      if (sender?.id_type === "app_id" && sender?.id === cfg.hostApp) {
        ...
        tst.active = true;
        // Round-robin: host @-mentioned a member → daemon invokes that member.
        // (Previously this was handled by OpenClaw's native lark plugin
        // auto-firing; now daemon owns it so we can support multi-chat agents.)
        //
        // Match strategy: exact-name first across ALL members, fall back to
        // substring-includes only when no exact match exists. This avoids
        // the "Dev" bot grabbing an @ intended for "DevOps" just because the
        // first iteration order put Dev earlier (substring of DevOps).
        if (cfg.mode === "round-robin" && msg.mentions) {
          let matchedAppId = null;
          let matchedMention = null;
          // Pass 1: exact match on botName
          for (const mention of msg.mentions) {
            const exact = cfg.memberApps.find(appId => mention.name === apps[appId].botName);
            if (exact) { matchedAppId = exact; matchedMention = mention; break; }
          }
          // Pass 2: substring fallback (only if no exact match found anywhere)
          if (!matchedAppId) {
            for (const mention of msg.mentions) {
              const fuzzy = cfg.memberApps.find(appId => {
                const a = apps[appId];
                return a.botName && (mention.name || "").includes(a.botName);
              });
              if (fuzzy) { matchedAppId = fuzzy; matchedMention = mention; break; }
            }
          }
          if (matchedAppId) {
            onLog(`[trigger RR] tid=${tid.slice(-8)} | host @${apps[matchedAppId].role} (mention="${matchedMention.name}") → invoke member`);
            // await: prevent fire-and-forget races where pollThread keeps
            // iterating, encounters another host @-msg, and re-enters
            // invokeHost while this one is still mid-flight (mutex would
            // drop the second one silently).
            await invokeRoundRobinMember(chatId, tid, matchedAppId, msg.message_id);
          }
        }
        continue;
      }
...
// Resolve host bots' open_ids per chat (used by `pollMain` to match @-mentions).
// open_id resolution is best-effort: if it fails, daemon falls back to
// matching by botName alone, which works in practice because Feishu fills
// `mentions[].name` correctly even when ids are per-app-scoped.
//
// Returns a flat config + populates each chat's `hostOpenId` (possibly null).
// `apps[i].openId` or `chats[i].hostOpenId` in the user's config takes
// precedence over auto-resolution.
export async function resolveHostOpenIds(config, { onLog = console.log, onWarn = console.warn } = {}) {
  const flat = flatten(config);
  for (const cid of Object.keys(flat.chats)) {
    const c = flat.chats[cid];

    // Manual override (config-provided): chats[i].hostOpenId or apps[i].openId
    const userChat = config.chats.find(ch => ch.chatId === cid);
    const userApp = config.apps.find(a => a.agent === c.hostAgent);
    if (userChat?.hostOpenId) { c.hostOpenId = userChat.hostOpenId; onLog(`[resolve] ${cid.slice(-6)} host open_id from config: ${c.hostOpenId.slice(0, 12)}…`); continue; }
    if (userApp?.openId)      { c.hostOpenId = userApp.openId;      onLog(`[resolve] ${cid.slice(-6)} host open_id from apps[].openId: ${c.hostOpenId.slice(0, 12)}…`); continue; }

    // Auto-resolve via members API or message history.
    const result = await resolveBotOpenId(c.hostApp, userApp.appSecret, cid, c.hostBotName);
    if (result.openId) {
      c.hostOpenId = result.openId;
      onLog(`[resolve] ${cid.slice(-6)} host open_id via ${result.source}: ${c.hostOpenId.slice(0, 12)}…`);
    } else {
      c.hostOpenId = null;
      onWarn(
        `[resolve] ${cid.slice(-6)} could NOT resolve open_id for "${c.hostBotName}". ` +
        `Daemon will fall back to matching @-mentions by name (usually fine). ` +
        `To eliminate this warning: add apps[].openId or chats[].hostOpenId to config, ` +
        `or have any user @-mention "${c.hostBotName}" once in the chat (history-based resolver picks it up).`
      );
    }
  }
  return flat;
