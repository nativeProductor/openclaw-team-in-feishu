...
export function render(templateName, vars) {
  let tpl = fs.readFileSync(templatePathFor(templateName), "utf8");
  // Strip Mustache-style comment blocks {{!-- ... --}} BEFORE variable
  // substitution. Otherwise the EDITING NOTES header (which references
  // placeholder names like {{sharedTranscriptPath}} as metadata) ends up
  // in the rendered SOUL.md polluting the agent's prompt.
  tpl = tpl.replace(/\{\{!--[\s\S]*?--\}\}\n?/g, "");
  tpl = tpl.replace(/^\s+/, ""); // trim any leading whitespace left by stripped comment block
